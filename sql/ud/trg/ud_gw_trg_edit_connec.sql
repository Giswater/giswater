...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        INSERT INTO connec (connec_id,top_elev,"ymax",connecat_id,sector_id, code,n_hydrometer,demand, "state", annotation,"observ","comment", rotation, dma_id, soilcat_id,category_type,fluid_type,location_type,
					workcat_id, buildercat_id, builtdate, ownercat_id, adress_01,adress_02,adress_03,streetaxis_id,postnumber,descript,link,verified,the_geom,undelete,workcat_id_end, y1, y2,featurecat_id,feature_id,
					private_connecat_id,label_x,label_y,label_rotation,accessibility,diagonal)
                    ...
                    NEW.dma_id, NEW.soilcat_id, NEW.category_type, NEW.fluid_type, NEW.location_type, NEW.workcat_id, NEW.buildercat_id, NEW.builtdate,NEW.ownercat_id, NEW.adress_01, NEW.adress_02, NEW.adress_03,
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        SET connec_id=NEW.connec_id, top_elev=NEW.top_elev, ymax=NEW."ymax", connecat_id=NEW.connecat_id, sector_id=NEW.sector_id, code=NEW.code, n_hydrometer=NEW.n_hydrometer,demand=NEW.demand, "state"=NEW."state", 
			...
			adress_03=NEW.adress_03, streetaxis_id=NEW.streetaxis_id, postnumber=NEW.postnumber, descript=NEW.descript,rotation=NEW.rotation, link=NEW.link, verified=NEW.verified, the_geom=NEW.the_geom, 
			workcat_id_end=NEW.workcat_id_end,y1=NEW.y1,y2=NEW.y2,undelete=NEW.undelete,featurecat_id=NEW.featurecat_id,feature_id=NEW.feature_id,private_connecat_id=NEW.private_connecat_id, 
