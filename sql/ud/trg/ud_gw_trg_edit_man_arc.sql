	...
	BEGIN
		...
		END IF;
			...
			IF man_table='man_conduit' THEN
				...
				code=NEW.conduit_code, expl_id=NEW.expl_id, publish=NEW.publish, inventory=NEW.inventory, end_date=NEW.conduit_end_date, uncertain=NEW.uncertain, macrodma_id=NEW.macrodma_id
