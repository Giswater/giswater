    ...
    IF TG_OP = 'INSERT' THEN
        ...
        IF (NEW.gratecat_id IS NULL) THEN
            IF ((SELECT COUNT(*) FROM cat_grate) = 0) THEN
                PERFORM audit_function(152,780);
			END IF;
        END IF;
        ...
        IF (NEW.sector_id IS NULL) THEN
            IF ((SELECT COUNT(*) FROM sector) = 0) THEN
                PERFORM audit_function(115,780); 
            END IF;
            NEW.sector_id := (SELECT sector_id FROM sector WHERE ST_DWithin(NEW.the_geom, sector.the_geom,0.001) LIMIT 1);
            IF (NEW.sector_id IS NULL) THEN
                PERFORM audit_function(120,780); 
            END IF;
        END IF;
        ...
        IF (NEW.dma_id IS NULL) THEN
            IF ((SELECT COUNT(*) FROM dma) = 0) THEN
                PERFORM audit_function(125,780); 
            END IF;
            NEW.dma_id := (SELECT dma_id FROM dma WHERE ST_DWithin(NEW.the_geom, dma.the_geom,0.001) LIMIT 1);
            IF (NEW.dma_id IS NULL) THEN
                PERFORM audit_function(130,780); 
            END IF;
        END IF;
