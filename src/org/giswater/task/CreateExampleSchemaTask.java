...
public class CreateExampleSchemaTask extends SwingWorker<Void, Void> {
	...
	private boolean loadRaster(String schemaName, String rasterPath, String rasterName) {
		...
		if (MainDao.checkTableHasData(schemaName, "mdt")) {
			String msg = Utils.getBundleString("CreateExampleSchemaTask.dtm_already_loaded"); 
    ...
    public Void doInBackground() { 
		...
		if (status) {
			...
			if (MainDao.updateSchema()) {
				...
				status = false;
				MainDao.deleteSchema(schemaName);
				MainClass.mdi.showError(Utils.getBundleString("CreateExampleSchemaTask.error_update_project")); 
			}		
		}
		else {
			status = false;
			MainDao.deleteSchema(schemaName);
			MainClass.mdi.showError(Utils.getBundleString("CreateExampleSchemaTask.error_create_project")); 
		}
    ...
    public void done() {
    	...
    	if (status) {
    		...
    		MainClass.mdi.showError(Utils.getBundleString("CreateExampleSchemaTask.error_create_project2")); 
    	}
