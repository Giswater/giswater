...
//
{
    ...
    if ( LinkStats ) for ( j = 0; j < Nobjects[LINK]; j++ )
    {
		...
		LinkStats[j].minShearStress = 10000000.0;
...
//
{
	...
	shearStress = Link[j].shearStress;
	if (shearStress > LinkStats[j].maxShearStress)
	{
		LinkStats[j].maxShearStress = shearStress;
		LinkStats[j].maxShearStressDate = aDate;
	}

	if (shearStress < LinkStats[j].minShearStress)
	{
		LinkStats[j].minShearStress = shearStress;
		LinkStats[j].minShearStressDate = aDate;
	}
