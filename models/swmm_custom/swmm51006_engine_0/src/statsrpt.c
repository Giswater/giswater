...
//
{
    ...
    fprintf(Frpt.file,
"\n  --------------------------------------------------------------------------------------------------------------------------"
"\n                                 Maximum  Time of Max   Maximum    Max/    Max/  Maximum  Time of Max   Minimum  Time of Min"
"\n                                  |Flow|   Occurrence   |Veloc|    Full    Full   Shear    Occurrence    Shear    Occurrence");
    if ( UnitSystem == US ) fprintf(Frpt.file,
"\n  Link                 Type          %3s  days hr:min    ft/sec    Flow   Depth  lbf/ft2  days hr:min   lbf/ft2  days hr:min",
        FlowUnitWords[FlowUnits]);
    else fprintf(Frpt.file, 
"\n  Link                 Type          %3s  days hr:min     m/sec    Flow   Depth     N/m2  days hr:min      N/m2  days hr:min",
        FlowUnitWords[FlowUnits]);
    fprintf(Frpt.file,
"\n  -----------------------------------------------------------------------------");
    ...
    for ( j = 0; j < Nobjects[LINK]; j++ )
    {
		...
		// --- print max. shear stress & time of occurrence
		getElapsedTime(LinkStats[j].maxShearStressDate, &days, &hrs, &mins);
		fprintf(Frpt.file, FlowFmt, LinkStats[j].maxShearStress*UCF(MASS)/UCF(LENGTH));
		fprintf(Frpt.file, "  %4d  %02d:%02d", days, hrs, mins);

		// --- print min. shear stress & time of occurrence
		getElapsedTime(LinkStats[j].minShearStressDate, &days, &hrs, &mins);
		fprintf(Frpt.file, FlowFmt, LinkStats[j].minShearStress*UCF(MASS) / UCF(LENGTH));
		fprintf(Frpt.file, "  %4d  %02d:%02d", days, hrs, mins);
