...
//
{
	...
	// --- read topo raster to interpolate polygon elevations
	input_readTopo();
...
//
{
	...
	// --- get INP path
	_splitpath(Finp.name, NULL, Finp.directory, Finp.fileName, NULL);

	// --- set filenames
	strcpy(F2Dmesh.name, Finp.directory);
	strcat(F2Dmesh.name, "2D_mesh");

	strcpy(Ftopo.name, Finp.directory);
	strcat(Ftopo.name, "topo.txt");

	strcpy(F2Doutflow.name, Finp.directory);
	strcat(F2Doutflow.name, "Outflow.txt");
...
//
{
	...
	// Fill 2D model controls
	M2DControl.maxNumFaces = Nobjects[M2DFACE];
    M2DControl.numFaces = 0;
	M2DControl.numElements = 0;
    ...
    {
        M2DFace[j].leftType   = POL_NONE;
        M2DFace[j].rightType  = POL_NONE;
...
void project_createShapefile()
//
//  Input:   none
//  Output:  none
//  Purpose: creates 2D mesh shapefile.
//
{

	//2D shapefile variables
	SHPHandle	hSHP;
	DBFHandle	hDBF;
	int		nShapeType;
	FILE *fptr;

	// --- open 2D mesh file (shapefile)
	nShapeType = SHPT_POLYGONZ;
	hSHP = SHPCreate(F2Dmesh.name, nShapeType);
	if( hSHP == NULL )
	{
		//TODO printf( "Unable to create:%s\n", argv[1] );
		//TODO exit( 3 );
	}
	SHPClose( hSHP );

	hDBF = DBFCreate(F2Dmesh.name);
	if( hDBF == NULL )
	{
		//TODO printf( "DBFCreate(%s) failed.\n", argv[1] );
		//TODO exit( 2 );
	}

	// --- Add ID field
    if( DBFAddField( hDBF, "ID", FTString, 50, 0 ) == -1 )
    {
		//TODO
	...
	fptr = fopen(F2Doutflow.name, "w");

	if (fptr == NULL)
	{
		printf("ERROR: Impossible to create Outflow.txt\n");
	} else {
		const char *text = "  Time (s)     flow (m3/s)";
		fprintf(fptr, "%s\n", text);
