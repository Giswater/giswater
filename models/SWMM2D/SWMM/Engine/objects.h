...
typedef struct
{
   char          name[MAXFNAME+1];     // complete file name
   char          fileName[MAXFNAME+1]; // file name
   char          directory[MAXFNAME+1];// file name
...
typedef struct
{
   ...
   TXYZ          centroid;        // polygon centroid

   int           isStreet;        // Is a roof or street polygon
   double        oldGlobalDepth;  // 2D depth (ft)
   double        newGlobalDepth;  // 2D depth (ft)

   double        oldVx;           // X velocity (ft/s)
   double        oldVy;           // Y velocity (ft/s)
   double        oldVel;
   double        newVx;           // X velocity (ft/s)
   double        newVy;           // Y velocity (ft/s)
   double        newVel;
   double        new_vh;          // unitary discharge modulus (ft2/s)

   double        oldqx;           // X momentum (ft2/s)
   double        oldqy;           // Y momentum (ft2/s)
   double        newqx;           // X momentum (ft2/s)
   double        newqy;           // Y momentum (ft2/s)

   double        Sx;              // slope X (ft/ft)
   double        Sy;              // slope Y (ft/ft)
   int           dbf_record;      // shapefile row number

   double        fluxDepth;       // flux h
   double        fluxHUx;         // flux hu 
   double        fluxHUy;         // flux hv

   double        shearStress;     // shear stress (lb/ft2)
   double        N;               // Manning coefficient for the subcatchment

   int           isIsolated;      // isolated polygon?
...
typedef struct
{
   ...
   double        M2DArea;         // 2D area connected to the node
   double        M2DDepth;        // 2D depth after 2D routing
}  TNode;
...
{
   //Static data
   char*         polygonLeftID;    // Left side interface polygon ID
   char*         polygonRightID;   // Right side interface polygon ID
   int           leftType;         // Left polygon building or street or boundary 
   int           rightType;        // Right polygon building or street or boundary
   double        X1;               
   double        Y1;
   double        X2;
   double        Y2;
...
//------------------
typedef struct {
	int             maxNumFaces;              // subcatchment name
	int             numFaces;
	int             numElements;              // number of polygons
	double			totalOutflow;             // total outflow through the boundary weirs
} T2DControl;
