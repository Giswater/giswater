...
//  subcatch_setOldHydState    (called from routing.c);
...
//
{
    ...
    Subcatch[j].area        = MAX(x[3] / UCF(LANDAREA),1.0);
...
//           ntoks = number of tokens
//  Output:  returns an error code
//  Purpose: reads 2D face parameters from a tokenized  line of input data.
//
//  Data has format:
//    Subcatchment  X  Y  
//
{
	...
	if (strcmp(TopoGrid.filename,"null") != 0)
	{
		x[2] = IntBilinear(x[0], x[1]);
	}else {
		x[2] = 0.0;
	}
	...
	if (!Polygon2D.ID)
	{
		...
		Polygon2D.vertex[Polygon2D.numVertex].x = x[0];
        Polygon2D.vertex[Polygon2D.numVertex].y = x[1];
		Polygon2D.vertex[Polygon2D.numVertex].z = x[2];
	...
	// Check if it is a point of a new polygon
	if (samestr(Polygon2D.ID, id) == 1) 
	{
...
//
{
    ...
    Subcatch[j].oldGlobalDepth = 0.0;
    Subcatch[j].newGlobalDepth = 0.0;

    Subcatch[j].oldVx = 0.0;
    Subcatch[j].oldVy = 0.0;
    Subcatch[j].oldVel = 0.0;
    Subcatch[j].newVx = 0.0;
    Subcatch[j].newVy = 0.0;
    Subcatch[j].newVel = 0.0;

    Subcatch[j].oldqx = 0.0;
    Subcatch[j].oldqy = 0.0;
...
void subcatch_setOldHydState(int j)
//
//  Input:   j = subcatchment index
//  Output:  none
...
//
{
	...
	Subcatch[j].oldVy = Subcatch[j].newVy;
	Subcatch[j].oldVel = Subcatch[j].newVel;

	// --- update 2D momentum
	Subcatch[j].oldqx = Subcatch[j].newqx;
	Subcatch[j].oldqy = Subcatch[j].newqy;
...
//  Output   face index
//  Purpose: checks shared face between polygons.
//
{
	//Variables
	int    numFaces;
	int    j;
	double vertexLength1, vertexLength2;


	//Code

	//Defined faces
	numFaces = M2DControl.numFaces;

	//Loop for pending faces
	for (j = 0; j < numFaces; j++)
	{
		if ( M2DFace[j].rightType == POL_NONE || M2DFace[j].leftType == POL_NONE)
		{
			//Check distances (in both senses)
			vertexLength1  = fabs(M2DFace[j].X1 - v1.x) + fabs(M2DFace[j].Y1 - v1.y);
			vertexLength1 += fabs(M2DFace[j].X2 - v2.x) + fabs(M2DFace[j].Y2 - v2.y);

			vertexLength2  = fabs(M2DFace[j].X1 - v2.x) + fabs(M2DFace[j].Y1 - v2.y);
			vertexLength2 += fabs(M2DFace[j].X2 - v1.x) + fabs(M2DFace[j].Y2 - v1.y);

			//The tolerance is 1 mm
			if ((vertexLength1 < 0.004) || (vertexLength2 < 0.004))
			{
				return j;
			}
		}
...
double IntBilinear(double X, double Y)
//
//  Input:   X = X coordinate of the point 
//           Y = Y coordinate of th epoint
//  Output   Z interpolated
//  Purpose: Interpolate Z coordinate of the polygon points.
//
{
	//Variables
    int i,j;
	...
	// Assign topo
	topo = (float **) TopoGrid.data;

	// Index of the cell containing the point
	i = (int)round( (X - TopoGrid.bndbox[0] - 0.5 * TopoGrid.dx) / TopoGrid.dx );
    j = (int)round( (Y - TopoGrid.bndbox[1] - 0.5 * TopoGrid.dy) / TopoGrid.dy );

	// Check out of domain
	if(i<0 || j<0 || j>TopoGrid.ny || i>TopoGrid.nx)
	{
        return TopoGrid.nodata;

	} else {

        resX = (X - TopoGrid.bndbox[0] - 0.5 * TopoGrid.dx) - TopoGrid.dx * (i - 1);
        resY = (Y - TopoGrid.bndbox[1] - 0.5 * TopoGrid.dy) - TopoGrid.dy * (j - 1);

        interpolation = topo[i][j]     * (TopoGrid.dx-resX) * (TopoGrid.dy-resY) / (TopoGrid.dx*TopoGrid.dy) + 
                               topo[i+1][j]   *   resX    * (TopoGrid.dy-resY) / (TopoGrid.dx*TopoGrid.dy) + 
                               topo[i][j+1]   * (TopoGrid.dx-resX) *   resY    / (TopoGrid.dx*TopoGrid.dy) + 
                               topo[i+1][j+1] *   resX    *   resY    / (TopoGrid.dx*TopoGrid.dy);
