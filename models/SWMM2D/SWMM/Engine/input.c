...
//
{
    ...
    {
		if (Node[i].M2DArea != 0.0)
		{
			Node[i].pondedArea = Node[i].M2DArea;
			Node[i].surDepth = 10000;
		}
    }
	...
	// -- check isolated polygons
	for (i = 0; i < Nobjects[M2DFACE]; i++)
	{
		subcatchLeft = NULL;
		subcatchRight = NULL;

		if (M2DFace[i].leftType == POL_STREET && M2DFace[i].rightType == POL_STREET)
		{
			subcatchLeft = project_findObject(SUBCATCH, M2DFace[i].polygonLeftID);
			subcatchRight = project_findObject(SUBCATCH, M2DFace[i].polygonRightID);

			Subcatch[subcatchLeft].isIsolated = 0;
			Subcatch[subcatchRight].isIsolated = 0;
		}
	}
...
//
{
    ...
    switch( objType )
    {
      ...
      case s_POLYGON:
//        if ( !project_addObject(M2DFACE, id, Nobjects[M2DFACE]) )
//            errcode = error_setInpError(ERR_DUP_NAME, id);
        Nobjects[M2DFACE]++;
        break;
...
//
{
    ...
    switch (sect)
    {
      ...
      case s_POLYGON:
        return polygon_readVertex(Tok, Ntokens);
...
void input_readTopo(void)
{
	...
	//Set raster filename
	sprintf(TopoGrid.filename, "%s", Ftopo.name);

	//Read raster
	err = gridread(TopoGrid.filename, &(TopoGrid.data), &(TopoGrid.nx), &(TopoGrid.ny), &(TopoGrid.dx), &(TopoGrid.dy), TopoGrid.bndbox, &(TopoGrid.nodata));
	if (err == 1) sprintf(TopoGrid.filename,"%s","null");
