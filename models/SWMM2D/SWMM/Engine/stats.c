...
void stats_updateCriticalTimeCount(int node, int link, int mesh)
//
...
//
{
	...
	else if ( mesh >= 0 ) SubcatchStats[mesh].timeCourantCritical += 1.0;
}
...
//
{
    ...
    delta = fabs(newDepth - Node[j].oldDepth);
...
//
{
    ...
    else if ( Link[j].type == CONDUIT )
    {
        ...
        if ( Link[j].newDepth >= Link[j].xsect.yFull )
        {
            LinkStats[j].timeSurcharged += tStep;
            LinkStats[j].timeFullUpstream += tStep;
            LinkStats[j].timeFullDnstream += tStep;
        }
        else if ( Conduit[k].a1 >= Link[j].xsect.aFull )
            LinkStats[j].timeFullUpstream += tStep;
        else if ( Conduit[k].a2 >= Link[j].xsect.aFull )
            LinkStats[j].timeFullDnstream += tStep;
    }
...
void M2D_maximums_print(void)
{
    ...
    for ( j=0; j<Nobjects[SUBCATCH]; j++)
    {
		if (Subcatch[j].isStreet)
		{
			...
			value = SubcatchStats[j].maxDepth * UCF(LENGTH);

			//Write value
			DBFWriteDoubleAttribute(hDBF, Subcatch[j].dbf_record, numFields - 3, value );

			value = SubcatchStats[j].maxVel * UCF(LENGTH);

			//Write value
			DBFWriteDoubleAttribute(hDBF, Subcatch[j].dbf_record, numFields - 2, value );

			value = SubcatchStats[j].max_vh * UCF(LENGTH) * UCF(LENGTH);
