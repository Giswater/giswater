...
static double get2DStep(double tMin, int *min2D);
//=============================================================================
...
//
{
	...
	int    min2D = -1;                  // index of element w/ min. time step
	...
	stats_updateCriticalTimeCount(minNode, minLink, min2D);
...
double get2DStep(double tMin, int *min2D)
//
//  Input:   tMin = critical time step found so far (sec)
//  Output:  min2D = index of 2d mesh with critical time step;
//           returns critical time step (sec)
//  Purpose: finds critical time step for 2D based on celerity.
//
{
	int    i;                           // node index
	double maxLambda;                    // max. depth allowed at node (ft)
	double t1;                          // time needed to reach depth limit (sec)
	double t2D = tMin;                // critical node time step (sec)
	...
	{
		// --- see if node can be skipped
		if (Subcatch[i].isStreet == 0) continue;
		if (Subcatch[i].newGlobalDepth <= FUDGE) continue;

		// --- compute maximum characteristic velocity
		maxLambda = fabs(Subcatch[i].newVel) + sqrt(GRAVITY*Subcatch[i].newGlobalDepth);

		// --- compute time to reach max. depth & compare with critical time
		if (maxLambda <= FUDGE) continue;

		// --- compute time to cross cell
		t1 = 0.5 * sqrt(Subcatch[i].area) / maxLambda  * CourantFactor;
